...
const CartPage = (): JSX.Element => {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12">
      <h1 className="text-3xl font-semibold text-black dark:text-secondary">Je winkelwagen</h1>
      {items.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-border p-12 text-center text-sm text-muted dark:border-border dark:text-muted">
        ...
        <div className="mt-8 grid gap-10 md:grid-cols-[2fr_1fr]">
                ...
                <div
                  ...
                  className={`rounded-2xl border p-6 shadow-sm ${
                    !isAvailable
                      ...
                      : 'border-border bg-white dark:border-border dark:bg-primary'
                  }`}
                >
                  <div className="flex items-start gap-6">
                    <img src={item.image} alt={item.title} className="h-24 w-24 rounded-xl object-cover" />
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-black dark:text-secondary">{item.title}</h3>
                      {hasListing && (
                        <div className="mt-2 flex items-center gap-2 text-sm text-muted dark:text-muted">
                      ...
                      {hasListing && (
                        <p className="mt-1 text-sm font-semibold text-black dark:text-secondary">
                          € {listingDetail.price.toFixed(2)}
                        </p>
                      )}
                      {!hasListing && listingsLoading && (
                        <p className="mt-1 text-sm text-muted dark:text-muted">Beschikbaarheid controleren...</p>
                      )}
                      {!hasListing && !listingsLoading && (
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        <label
                          ...
                          className="text-xs uppercase tracking-wide text-muted dark:text-muted"
                        >
                          Aantal
                        </label>
                        <Input
                    ...
                    <button
                      type="button"
                      className="text-muted transition hover:text-red-500 dark:text-muted"
          ...
          <div className="rounded-2xl border border-border bg-white p-6 shadow-sm dark:border-border dark:bg-primary">
            <h2 className="text-xl font-semibold text-black dark:text-secondary">Overzicht</h2>
            <div className="mt-4 space-y-3 text-sm text-muted dark:text-muted">
              <div className="flex justify-between">
                <span>Subtotaal</span>
                <span className="text-black dark:text-secondary">€ {total.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span>Verzending</span>
                <span className="text-black dark:text-secondary">Gratis</span>
              </div>
            </div>
            <div className="mt-6 flex justify-between text-lg font-semibold text-black dark:text-secondary">
              <span>Totaal</span>
